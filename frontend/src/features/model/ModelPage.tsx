...
export function ModelPage() {
  ...
  return <div className="workflow-page"><header className="workflow-heading"><div><h1>模型评估</h1><p>候选模型经训练集交叉验证选择，并在独立保留集完成最终评估。</p></div><StatusBadge status={run.status} /></header><section className="model-summary"><div><span>入选模型</span><strong>{label(modelResult.model_family)}</strong><small>{label(modelResult.strategy)}</small></div><div><span>{firstMetric?.[0]?.toUpperCase().replaceAll("_", "-") ?? "评估指标"}</span><strong>{typeof firstMetric?.[1] === "number" ? firstMetric[1].toLocaleString("zh-CN", { maximumFractionDigits: 4 }) : "-"}</strong><small>保留集 {Number(split.test_rows ?? 0).toLocaleString("zh-CN")} 行</small></div><div><span>泄露控制</span><strong><CheckCircle2 size={19} /> {leakage.test_used_for_selection === false ? "已通过" : "待确认"}</strong><small>{label(leakage.model_selection_scope ?? leakage.fit_scope)}</small></div><div><span>数据版本</span><strong>{run.dataset_version_id}</strong><small>训练集 {Number(split.train_rows ?? 0).toLocaleString("zh-CN")} 行</small></div></section><ArtifactViewer artifacts={artifacts} /></div>;
}
