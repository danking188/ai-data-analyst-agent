  ...
  return (
    <main className="login-page">
      <form className="login-panel" onSubmit={submit}>
        ...
        <label className="field"><span>用户名</span><input aria-describedby={mode === "register" ? "registration-username-hint" : undefined} aria-label="用户名" autoCapitalize="none" autoComplete="username" autoFocus maxLength={32} pattern="[A-Za-z0-9][A-Za-z0-9_.\x2d]{2,31}" required value={username} onChange={(event) => setUsername(event.target.value)} />{mode === "register" ? <small id="registration-username-hint">3-32 位，可使用字母、数字、点、下划线和连字符。</small> : null}</label>
