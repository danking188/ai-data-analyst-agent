...
截至 **2026-09-16**，阿里云新加坡轻量服务器上的小规模版本已正式上线，公网就绪检查正常，
自助注册和真实 AI Agent 已开启。最新完整验收包括：

- 新账号完成注册、自动登录、账号隔离、退出重登，并完成上传、质量扫描、模型训练、报告下载。
- 真实 Qwen Agent 完成证据解读、只读工具调用、分析规划、确认前阻断及确认后的真实建模执行；
  3 个轮次、5 次工具调用全部成功。
- 真实模型 Agent 分层抽样评测 8/8 通过：任务成功率与硬规则通过率均为 100%，
  P95 延迟 21.827 秒，未授权写操作为 0。
- 后端 168 项、前端 33 项测试通过，后端覆盖率 83%，类型检查、OpenAPI 与生产构建通过。
- 服务器侧 500 请求、并发 8：0 失败，P95 228.934 ms，吞吐 50.612 req/s；该指标为轻量读取探针，
  不代表 8 个并发分析任务或大文件容量。
...
访问延迟会受线路影响。LLM 使用阿里云百炼兼容接口与 `qwen3.8-flash`；模型不可用时，
