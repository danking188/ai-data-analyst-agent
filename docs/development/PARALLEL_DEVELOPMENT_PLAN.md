...
> 历史基线：本文件记录项目早期前后端并行开发过程，不再作为当前发布状态事实源。
> 当前任务、验证证据和生产环境待办统一维护在
...
完整架构、数据模型、API 草案、任务拆分、测试与发布门禁见
[`LLM_EXTENSION_PLAN.md`](LLM_EXTENSION_PLAN.md)。本阶段继续坚持“LLM 负责理解、规划和叙述，
确定性工具负责计算，Artifact/Claim 负责证据”的边界。

| ID | 状态 | 任务 | 完成条件 |
|---|---|---|---|
| LLM-P0 | [x] | Provider、配置、结构化输出与故障处理 | LLM 关闭时全功能回归通过；Fake/真实 Provider 可替换；密钥不进入日志和数据库 |
| LLM-P1 | [x] | 证据型 AI 解读 | 真实 Run 可生成业务摘要；所有数字结论绑定有效 Artifact/Claim |
| LLM-P2 | [x] | 对话式分析与受约束规划 | 多轮会话、只读工具、分析计划、Job、取消和重试端到端贯通 |
| LLM-P3 | [x] | 清洗、分析与建模 Copilot | 可生成可编辑草案；执行、高成本或版本变更操作必须显式确认 |
| LLM-P4 | [x] | 受控自主分析与生产强化 | 多步执行受工具白名单、Token/调用预算、熔断、审计和安全评估约束 |

以上状态已按 `LLM_EXTENSION_PLAN.md` 的最终实施与验收记录回填；本文件仍只作为历史任务基线，
当前发布结论以 `PROJECT_STATUS.md` 为准。

历史首个执行批次（已完成）：

```text
LLM-SH-001 → LLM-SH-002 → LLM-BE-001 → LLM-BE-002
→ LLM-BE-004 → LLM-BE-003 → LLM-QA-001 → Gate L0
```
